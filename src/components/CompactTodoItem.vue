  ...
  >
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex items-center gap-1.5 mt-1">
        ...
        <span v-if="(todo.tags || []).length > 2" class="text-[10px] text-content-tertiary dark:text-gray-500">
          +{{ todo.tags.length - 2 }}
        </span>
        <!-- Step progress -->
    ...
    <div class="flex items-center gap-0.5">
      <button @click="$emit('edit', todo)" class="p-1 rounded-lg hover:bg-surface-tertiary dark:hover:bg-gray-700 text-content-tertiary hover:text-content dark:hover:text-gray-300 transition-colors">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
        </svg>
      </button>
      <button @click="$emit('delete', todo)" class="p-1 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-content-tertiary hover:text-red-500 transition-colors">
        <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const statusClass = computed(() => {
  ...
  if (props.todo.status === 'in_progress') return 'border-blue-400 bg-blue-50 dark:bg-blue-900/20'

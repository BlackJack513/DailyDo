<template>
 <aside class="w-56 flex-shrink-0 border-r border-border flex flex-col bg-sidebar h-full" :class="{ 'bg-opacity-90': store.isMiniMode }">
 <!-- Logo -->
...
>
 ...
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6a2 2 0 012-2h8a2 2 0 012 2v8a2 2 0 01-2 2H6a2 2 0 01-2-2V6z" />
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 5l4-2v12l-4-2" />
 </svg>
 <svg v-else class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18H18M6 18L4 20M6 18L4 16M18 6l2-2M18 6l2 2M18 6L16 4M18 6l-2 2" />
 </svg>
 ...
 <div class="relative">
   <button class="sidebar-item w-full" @click="showThemeDropdown = !showThemeDropdown">
     ...
     <span class="w-3 h-3 rounded-full flex-shrink-0 border border-white/20" :style="{ backgroundColor: currentThemeColor }"></span>
   </button>
   <div v-if="showThemeDropdown" class="absolute bottom-full left-0 right-0 mb-1 bg-surface border border-border rounded-xl shadow-lg overflow-hidden z-50">
     <button
     ...
     >
       <span class="w-3 h-3 rounded-full flex-shrink-0 border border-black/10" :style="{ backgroundColor: t.color }"></span>
       <span>{{ t.name }}</span>
       <svg v-if="store.theme === t.id" class="w-4 h-4 ml-auto text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">

<template>
  <aside class="w-56 flex-shrink-0 border-r border-border dark:border-gray-700 flex flex-col bg-surface-secondary dark:bg-gray-850 h-full" :class="{ 'bg-opacity-90 dark:bg-opacity-90': store.isMiniMode }">
    <!-- Logo -->
    <div class="px-5 py-5 flex items-center gap-2.5">
      ...
      <span class="text-lg font-bold text-content dark:text-gray-100">DailyDo</span>
    </div>
    ...
    <!-- Bottom actions -->
    <div class="px-3 py-4 border-t border-border dark:border-gray-700 space-y-1">
      <!-- Sidebar config -->
      ...
      <!-- Theme toggle -->
      <button class="sidebar-item w-full" @click="store.toggleTheme()">
        ...
        <span>{{ store.theme === 'light' ? '深色模式' : '浅色模式' }}</span>
      </button>
    </div>

    <!-- Status bar -->
    <div class="px-5 py-3 border-t border-border dark:border-gray-700">
      <p class="text-xs text-content-tertiary dark:text-gray-500">

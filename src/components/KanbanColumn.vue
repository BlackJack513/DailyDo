<template>
  <div
    ref="columnEl"
    data-kanban-column
    class="flex flex-col rounded-xl border-t-4 bg-surface-secondary/50 dark:bg-gray-800/50 border-border dark:border-gray-700 overflow-hidden transition-all duration-300 flex-1 min-w-[280px]"
    :class="[statusColor, isDragOver ? 'ring-2 ring-primary/40 dark:ring-primary/60' : '']"
  >
    <!-- Column Header -->
    <div
      class="px-4 py-3 flex items-center justify-between border-b border-border dark:border-gray-700 cursor-pointer transition-colors select-none"
      :class="isDragOver ? 'bg-primary/10 dark:bg-primary/20' : ''"
      @click="collapsed = !collapsed"
    >
      <div class="flex items-center gap-2">
        <span class="text-sm font-semibold text-content dark:text-gray-200">{{ title }}</span>
    ...
    >
      <div
        ...
        class="group flex items-start gap-3 rounded-xl border border-border dark:border-gray-700 hover:border-primary/30 dark:hover:border-indigo-500/30 transition-all duration-150 bg-surface dark:bg-gray-800 select-none"
        :class="[
          todo.steps && todo.steps.length > 0 ? 'p-4' : 'p-3',
          {
            ...
            'cursor-grabbing': dragState && dragState.todo.id === todo.id
          }
        ]"
      >
        <!-- Status toggle -->
        ...
        <div class="flex-1 min-w-0">
          ...
          <div class="flex items-center gap-2 mt-1.5 flex-wrap">
            ...
            <span v-if="todo.recurrence_type && todo.recurrence_type !== 'none'" class="text-xs text-content-tertiary dark:text-gray-500 flex items-center gap-0.5">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
              </svg>
              {{ getRecurrenceLabel(todo.recurrence_type) }}
            </span>
            <span v-if="todo.attachment_path" class="text-xs text-content-tertiary dark:text-gray-500 flex items-center gap-0.5">
              <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div v-if="todo.steps && todo.steps.length > 0" class="mt-3 space-y-1.5">
            ...
            <div
              ...
              class="flex items-center gap-2 py-1 px-2 rounded-lg cursor-pointer transition-colors hover:bg-surface-secondary dark:hover:bg-gray-700/50"
            >
              <div
              ...
              <span
                ...
                :class="step.completed
                  ? 'line-through text-content-tertiary dark:text-gray-500'
        ...
        <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
          <button @click.stop="$emit('edit', todo)" class="p-1.5 rounded-lg hover:bg-surface-tertiary dark:hover:bg-gray-700 text-content-tertiary hover:text-content dark:hover:text-gray-300 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
            </svg>
          </button>
          <button @click.stop="$emit('delete', todo)" class="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-content-tertiary hover:text-red-500 transition-colors">
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <!-- Empty column hint -->
      <div v-if="todos.length === 0" class="flex flex-col items-center justify-center py-8 text-content-tertiary dark:text-gray-600">
    ...
    >
      <div class="flex items-start gap-3 p-3 rounded-xl border-2 border-primary bg-surface dark:bg-gray-800 shadow-2xl min-w-[250px] max-w-[320px]">
        <div class="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center" :class="getStatusClass(dragState.todo.status)">
          <svg v-if="dragState.todo.status === 'done'" class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
            <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
          </svg>
          <div v-else-if="dragState.todo.status === 'in_progress'" class="w-2 h-2 rounded-full bg-primary"></div>
        </div>
        <div class="flex-1 min-w-0">
          <p class="text-sm font-medium leading-tight text-content dark:text-gray-100 truncate">{{ dragState.todo.title }}</p>
...
function getStepProgressClass(todo) {
  if (!todo.steps || todo.steps.length === 0) return 'text-content-tertiary dark:text-gray-500'
  ...
  return 'text-content-tertiary dark:text-gray-500'
}
...
function getStepBarColor(todo) {
  if (!todo.steps || todo.steps.length === 0) return 'bg-gray-300 dark:bg-gray-600'
  const done = todo.steps.filter(s => s.completed).length

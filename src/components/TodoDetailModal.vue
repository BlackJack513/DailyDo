<template>
 ...
 <!-- Modal -->
 <div class="relative w-full max-w-xl mx-4 bg-surface rounded-2xl shadow-2xl border border-divider overflow-hidden animate-in">
 <!-- Header -->
 <div class="flex items-center justify-between px-6 py-4 border-b border-divider">
 <h3 class="text-base font-semibold text-body text-content">待办详情</h3>
 <button @click="$emit('close')" class="p-1.5 rounded-lg hover:bg-surface-2 text-muted hover:text-content-secondary transition-colors">
 ...
 <!-- Title -->
 <h2 class="text-lg font-bold text-body text-content leading-relaxed">{{ todo?.title }}</h2>
 ...
 <span v-if="todo?.due_date" class="inline-flex items-center gap-1 text-xs text-muted">
 ...
 <span v-if="todo?.recurrence_type && todo.recurrence_type !== 'none'" class="inline-flex items-center gap-1 text-xs text-muted">
 <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
>
 ...
 <p class="text-xs font-medium text-muted uppercase tracking-wider">任务步骤</p>
 <span class="text-xs text-muted">{{ stepCompleted }}/{{ todo.steps.length }} 已完成</span>
 </div>
 <!-- Progress bar -->
...
>
 ...
 :class="step.completed ? 'bg-green-500 border-green-500' : 'border-divider group-hover:border-primary'"
>
 <svg v-if="step.completed" class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20">
 <path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd" />
 </svg>
 </span>
 <span class="text-sm" :class="step.completed ? 'text-muted line-through' : 'text-content-secondary'">
 {{ step.title }}
 </span>
 </button>
 </div>
 ...
 <p class="text-xs font-medium text-muted uppercase tracking-wider">备注</p>
 <div
 class="p-3.5 rounded-xl bg-surface-2 text-content-secondary text-content text-sm leading-relaxed prose prose-sm prose-invert max-w-none"
 v-html="todo.notes"
></div>
 </div>
...
>
 ...
 <!-- Footer -->
 <div class="flex items-center justify-between px-6 py-3.5 border-t border-divider bg-surface-2/80 bg-surface/80">
 ...
 <button @click="$emit('delete', todo)" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-medium text-red-500 hover:bg-red-50 hover:bg-red-50/20 transition-colors">
 <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const statusClass = computed(() => {
 const map = {
 pending: 'bg-amber-50 text-amber-600 text-amber-400',
 in_progress: 'bg-blue-50 text-blue-600 text-blue-400',
 done: 'bg-emerald-50 bg-emerald-50/20 text-emerald-600 text-emerald-400',
 }
...
const priorityClass = computed(() => {
 const map = {
 high: 'bg-red-50 text-red-600 text-red-400',
 medium: 'bg-amber-50 text-amber-600 text-amber-400',
 low: 'bg-emerald-50 bg-emerald-50/20 text-emerald-600 text-emerald-400',
 }

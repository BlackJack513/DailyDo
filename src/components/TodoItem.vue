<template>
  <div
    ...
    class="group flex items-start gap-3 p-3 rounded-xl border border-border dark:border-gray-700 hover:border-primary/30 dark:hover:border-indigo-500/30 transition-all duration-150 bg-surface dark:bg-gray-800"
  ...
  >
    ...
    <div class="flex-1 min-w-0">
      ...
      <div class="flex items-center gap-2 mt-1.5 flex-wrap">
        ...
        <!-- Attachment -->
        <span v-if="todo.attachment_path" class="text-xs text-content-tertiary dark:text-gray-500 flex items-center gap-0.5" :title="todo.attachment_name || '附件'">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <span v-if="todo.steps && todo.steps.length > 0" class="text-xs flex items-center gap-1">
          <svg class="w-3 h-3 text-content-tertiary dark:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-6 9l2 2 4-4" />
          </svg>
          <span :class="stepProgressClass">{{ stepCompletedCount }}/{{ todo.steps.length }}</span>
        </span>
        <!-- Date -->
        <span v-if="showDate" class="text-xs text-content-tertiary dark:text-gray-500 flex items-center gap-0.5">
          <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
      <button @click="$emit('edit', todo)" class="p-1.5 rounded-lg hover:bg-surface-tertiary dark:hover:bg-gray-700 text-content-tertiary hover:text-content dark:hover:text-gray-300 transition-colors">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
        </svg>
      </button>
      <button @click="$emit('delete', todo)" class="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-content-tertiary hover:text-red-500 transition-colors">
        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
const stepProgressClass = computed(() => {
  if (!props.todo.steps || props.todo.steps.length === 0) return 'text-content-tertiary dark:text-gray-500'

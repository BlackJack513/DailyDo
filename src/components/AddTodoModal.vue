<template>
 ...
 :class="form.priority === p.value ? p.activeClass : 'border-border text-content-tertiary hover:border-content-tertiary'"
...
>
 ...
 :class="[ form.tagIds.includes(tag.id) ? 'border-current opacity-100' : 'border-transparent opacity-50 hover:opacity-75', isLocked('tags') ? 'cursor-not-allowed' : 'cursor-pointer' ]"
...
>
 ...
 :class="form.recurrence_type === r.value ? 'border-primary bg-primary/10 text-primary' : 'border-border text-content-tertiary hover:border-content-tertiary'"
...
>
 ...
 <!-- Attachment -->
 <div>
 <label class="block text-sm font-medium text-content-secondary text-muted mb-1.5">附件（单个，最大 10MB）</label>
 ...
 <button @click="removeAttachment" class="p-1 rounded hover:bg-red-50 hover:bg-red-50/20 text-red-400 hover:text-red-500">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
 </svg>
 </button>

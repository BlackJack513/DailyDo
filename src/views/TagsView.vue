    ...
    <div class="flex-1 px-8 pb-6 overflow-y-auto">
      ...
      <div class="mb-8">
        ...
        <div class="grid grid-cols-2 gap-3">
          <div v-for="tag in presetTags" :key="tag.id" class="card flex items-center justify-between">
      ...
      <div>
        ...
        <div v-else class="grid grid-cols-2 gap-3">
          <div v-for="tag in customTags" :key="tag.id" class="card flex items-center justify-between group">
            <div class="flex items-center gap-3">
              <div class="w-4 h-4 rounded-full" :style="{ backgroundColor: tag.color }"></div>
              <span class="text-sm font-medium text-content">{{ tag.name }}</span>
            </div>
            <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
              ...
              <button
                ...
                class="p-1.5 rounded-lg hover:bg-red-50 hover:bg-red-50/20 text-content-tertiary hover:text-red-500"
              >
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path
                    stroke-linecap="round"

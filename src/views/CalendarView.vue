<template>
  <div class="h-full flex flex-col">
    ...
    <div
      v-if="isManageMode"
      class="mx-8 mb-3 px-4 py-2.5 rounded-xl bg-primary/10 border border-primary/20 flex items-center gap-2"
    >
      ...
      <div class="grid grid-cols-7 gap-1.5">
        ...
        >
          ...
          <div v-if="day.date && day.isCurrentMonth" class="mt-1 flex items-center justify-center">
            <!-- Workday icon: sun -->
            <svg
              v-if="getDayType(day.date) === 'workday'"
          ...
          <div v-if="day.data" class="flex items-center gap-1 mt-0.5">
            <span
              ...
              :class="countBadgeClass(day.data, day.date)"
            >
          ...
          <div
            v-if="day.date && store.calendarDays[day.date]"
            class="absolute bottom-1 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full"
            :class="store.calendarDays[day.date] === 'workday' ? 'bg-blue-400' : 'bg-amber-400'"
          ></div>
        </div>
      </div>
        ...
        <div class="flex items-center justify-between px-6 py-4 border-b border-border">
          <div class="flex items-center gap-2">
            ...
            <span
              ...
              :class="
                selectedDateType === 'workday' ? 'bg-blue-500/10 text-blue-500' : 'bg-amber-500/10 text-amber-500'
              "
            >
        ...
        <div class="px-6 py-4 max-h-80 overflow-y-auto space-y-2">
          ...
          >
            <div
              ...
              :class="
                todo.status === 'done' ? 'bg-green-500' : todo.status === 'in_progress' ? 'bg-primary' : 'bg-control'
              "
            ></div>
            <span
              class="text-sm flex-1"
              :class="todo.status === 'done' ? 'line-through text-content-tertiary' : 'text-content'"
            >
              {{ todo.title }}
            </span>
...
function cellClasses(day) {
  ...
  const overrideRing = isManageMode.value && store.calendarDays[day.date] ? 'ring-2 ring-primary/30' : ''

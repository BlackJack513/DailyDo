<template>
  <div class="h-full flex flex-col">
    ...
    <div class="px-8 pt-6 pb-4 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-content dark:text-gray-100">日历视图</h1>
        <p class="text-sm text-content-tertiary dark:text-gray-500 mt-0.5">查看每天的待办完成情况，管理工作日与休息日</p>
      </div>
      <div class="flex items-center gap-3">
        ...
        <button
          ...
          :class="isManageMode
            ...
            : 'bg-surface-tertiary dark:bg-gray-700 text-content-secondary hover:bg-surface-hover dark:hover:bg-gray-600'"
        >
        ...
        <!-- Month navigation -->
        <button @click="prevMonth" class="p-2 rounded-lg hover:bg-surface-tertiary dark:hover:bg-gray-700 text-content-secondary">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" />
          </svg>
        </button>
        <span class="text-lg font-semibold text-content dark:text-gray-100 min-w-[140px] text-center">
          {{ currentYear }}年{{ currentMonth }}月
        </span>
        <button @click="nextMonth" class="p-2 rounded-lg hover:bg-surface-tertiary dark:hover:bg-gray-700 text-content-secondary">
          <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Manage mode hint -->
    <div v-if="isManageMode" class="mx-8 mb-3 px-4 py-2.5 rounded-xl bg-primary/10 dark:bg-indigo-500/10 border border-primary/20 dark:border-indigo-500/20 flex items-center gap-2">
      <svg class="w-4 h-4 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
      </svg>
      <span class="text-xs text-primary dark:text-indigo-300">管理模式下，点击日期可切换工作日与休息日。点击"完成管理"退出管理模式。</span>
    </div>
    ...
    <div class="flex-1 px-8 pb-6">
      ...
      <div class="grid grid-cols-7 mb-2">
        <div v-for="day in weekDays" :key="day" class="text-center text-xs font-semibold text-content-tertiary dark:text-gray-500 py-2">
          {{ day }}
        </div>
      </div>
      ...
      <div class="grid grid-cols-7 gap-1.5">
        ...
        >
          ...
          <div v-if="day.date && day.isCurrentMonth" class="mt-1 flex items-center justify-center">
            <!-- Workday icon: sun -->
            <svg v-if="getDayType(day.date) === 'workday'" class="w-3.5 h-3.5 text-blue-500 dark:text-blue-400" fill="currentColor" viewBox="0 0 24 24">
            ...
            <svg v-else class="w-3.5 h-3.5 text-amber-500 dark:text-amber-400" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div class="flex items-center gap-6 mt-6 justify-center flex-wrap">
        <div class="flex items-center gap-2">
          ...
          <span class="text-xs text-content-tertiary dark:text-gray-500">工作日</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-3 h-3 rounded-full bg-amber-500/20 border border-amber-500"></div>
          <span class="text-xs text-content-tertiary dark:text-gray-500">休息日</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-3 h-3 rounded-full bg-green-500/20 border border-green-500"></div>
          <span class="text-xs text-content-tertiary dark:text-gray-500">全部完成</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-3 h-3 rounded-full bg-red-500/20 border border-red-500"></div>
          <span class="text-xs text-content-tertiary dark:text-gray-500">逾期未完成</span>
        </div>
        <div class="flex items-center gap-2">
          <div class="w-3 h-3 rounded-full bg-surface-tertiary dark:bg-gray-700 border border-border dark:border-gray-600"></div>
          <span class="text-xs text-content-tertiary dark:text-gray-500">无待办</span>
        </div>
      </div>
    </div>
    ...
    <div v-if="selectedDate && !isManageMode" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative w-full max-w-md mx-4 bg-surface dark:bg-gray-800 rounded-2xl shadow-2xl border border-border dark:border-gray-700">
        <div class="flex items-center justify-between px-6 py-4 border-b border-border dark:border-gray-700">
          <div class="flex items-center gap-2">
            <h3 class="text-lg font-semibold text-content dark:text-gray-100">{{ selectedDate }} 的待办</h3>
          ...
          <button @click="selectedDate = null" class="p-1 rounded-lg hover:bg-surface-tertiary dark:hover:bg-gray-700 text-content-tertiary">
            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="px-6 py-4 max-h-80 overflow-y-auto space-y-2">
          <div v-if="selectedDateTodos.length === 0" class="text-center py-8 text-content-tertiary dark:text-gray-500 text-sm">
            当天没有待办事项
          </div>
          <div
            v-for="todo in selectedDateTodos"
            :key="todo.id"
            class="flex items-center gap-3 p-2.5 rounded-lg bg-surface-secondary dark:bg-gray-700/50"
          >
            <div class="w-2 h-2 rounded-full flex-shrink-0" :class="todo.status === 'done' ? 'bg-green-500' : todo.status === 'in_progress' ? 'bg-primary' : 'bg-border dark:bg-gray-600'"></div>
            <span class="text-sm flex-1" :class="todo.status === 'done' ? 'line-through text-content-tertiary' : 'text-content dark:text-gray-200'">{{ todo.title }}</span>
...
function cellClasses(day) {
  ...
  const manageHover = isManageMode.value
    ? 'hover:border-primary/50 dark:hover:border-indigo-400/50 hover:ring-2 hover:ring-primary/20'
    : 'hover:border-primary/30 dark:hover:border-indigo-500/30'

  // Completion-based classes
  let completionClass = ''
  if (day.data) {
    if (day.data.completed === day.data.total) {
      completionClass = 'bg-green-500/10 border-green-500/30 dark:bg-green-500/10 dark:border-green-500/30'
    } else if (day.data.completed > 0) {
      completionClass = 'bg-amber-500/10 border-amber-500/30 dark:bg-amber-500/10 dark:border-amber-500/30'
    } else {
    ...
    completionClass = dayType === 'rest'
      ...
      : 'bg-blue-50/50 dark:bg-blue-900/5 border-blue-200/50 dark:border-blue-800/20'
  }
  ...
  if (isOverdue) {
    completionClass = 'bg-red-500/20 border-red-500/50 dark:bg-red-500/25 dark:border-red-500/60'
  }

  // Manage mode: if custom override, add ring
  const overrideRing = isManageMode.value && store.calendarDays[day.date]
    ? 'ring-2 ring-primary/30 dark:ring-indigo-500/30'
    : ''
...
function countBadgeClass(data, dateStr) {
  ...
  if (dateStr && dateStr < todayStr.value && data.completed < data.total) {
    return 'bg-red-500/30 text-red-600 dark:text-red-400'
  }
  if (data.completed === data.total) return 'bg-green-500/20 text-green-600 dark:text-green-400'
  if (data.completed > 0) return 'bg-amber-500/20 text-amber-600 dark:text-amber-400'
  return 'bg-surface-tertiary dark:bg-gray-600 text-content-tertiary dark:text-gray-400'
}

...
>
 ...
 <!-- Empty state -->
 <div v-if="filteredAttachments.length === 0 && !loading" class="flex flex-col items-center justify-center py-20 text-content-tertiary">
...
>
 ...
 <p class="text-sm text-content-secondary text-muted mb-1">确定要删除附件「{{ deleteTarget.attachment_name }}」吗？</p>
 ...
 <p class="text-xs text-content-tertiary mb-1">将删除 <span class="font-semibold text-red-500">{{ completedCount }}</span> 个附件，释放约 <span class="font-semibold text-primary">{{ formatSize(completedSize) }}</span> 空间。</p>

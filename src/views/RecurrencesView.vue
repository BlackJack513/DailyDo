<template>
  <div class="h-full flex flex-col overflow-y-auto">
    ...
    <div class="px-8 pt-6 pb-4 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-content dark:text-gray-100">周期任务</h1>
    ...
    <div class="flex-1 px-8 pb-6">
      ...
      <div v-if="loading" class="flex items-center justify-center h-48">
        <p class="text-sm text-content-tertiary dark:text-gray-500">加载中...</p>
      </div>

      <!-- Empty -->
      <div v-else-if="groups.length === 0" class="flex flex-col items-center justify-center h-64 text-content-tertiary dark:text-gray-500">
      ...
      <div v-else class="space-y-3">
        <div
          ...
          :key="group.groupId"
          class="flex items-center gap-4 p-4 rounded-xl border border-border dark:border-gray-700 bg-surface-secondary/50 dark:bg-gray-800/50 hover:border-primary/30 dark:hover:border-indigo-500/30 transition-colors"
        >
          <!-- Recurrence icon -->
          <div class="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0" :class="recurrenceColor(group.recurrenceType)">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-semibold text-content dark:text-gray-100 truncate">{{ group.title }}</p>
            <div class="flex items-center gap-2 mt-0.5">
              <span class="text-xs px-2 py-0.5 rounded-full font-medium" :class="recurrenceBadgeClass(group.recurrenceType)">
                {{ recurrenceLabel(group.recurrenceType) }}
              </span>
              <span class="text-xs text-content-tertiary dark:text-gray-500">
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            <button
              ...
              :class="group.enabled ? 'bg-primary' : 'bg-gray-300 dark:bg-gray-600'"
              :title="group.enabled ? '点击关闭周期任务' : '点击开启周期任务'"
            >
              <span
                class="inline-block h-4 w-4 transform rounded-full bg-white transition-transform duration-200 shadow"
                :class="group.enabled ? 'translate-x-6' : 'translate-x-1'"
              />
            </button>
            <span class="text-xs font-medium w-8" :class="group.enabled ? 'text-primary' : 'text-content-tertiary dark:text-gray-500'">
              {{ group.enabled ? '开启' : '关闭' }}
            </span>
          </div>
          ...
          <div class="flex items-center gap-1 flex-shrink-0">
            <button @click="editGroup(group)" class="p-2 rounded-lg hover:bg-surface-tertiary dark:hover:bg-gray-700 text-content-tertiary hover:text-content dark:hover:text-gray-300 transition-colors" title="编辑">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
              </svg>
            </button>
            <button @click="deleteGroup(group)" class="p-2 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-content-tertiary hover:text-red-500 transition-colors" title="删除">
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
function recurrenceBadgeClass(type) {
  const map = {
    daily: 'bg-blue-50 dark:bg-blue-900/20 text-blue-600 dark:text-blue-400',
    weekly: 'bg-purple-50 dark:bg-purple-900/20 text-purple-600 dark:text-purple-400',
    monthly: 'bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400',
    yearly: 'bg-rose-50 dark:bg-rose-900/20 text-rose-600 dark:text-rose-400',
    workday: 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-600 dark:text-emerald-400',
  }
...
async function loadData() {
  ...
  try {
    ...
    allRecurring.value = todos.filter(

<template>
  <div class="h-full flex flex-col overflow-y-auto">
    ...
    <div class="px-8 pt-6 pb-4 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-content dark:text-gray-100">统计分析</h1>
        <p class="text-sm text-content-tertiary dark:text-gray-500 mt-0.5">了解你的工作效率和任务分布</p>
      </div>
      <div class="flex gap-2">
        <button
          ...
          :class="selectedRange === range.value ? 'border-primary bg-primary/10 text-primary' : 'border-border dark:border-gray-600 text-content-tertiary hover:border-content-tertiary'"
        >
          {{ range.label }}
        </button>
      </div>
    ...
    <div class="px-8 pb-6 grid grid-cols-4 gap-4">
      <div v-for="stat in statsCards" :key="stat.label" class="card">
        <p class="text-xs font-medium text-content-tertiary dark:text-gray-500 mb-1">{{ stat.label }}</p>
        <p class="text-2xl font-bold text-content dark:text-gray-100">{{ stat.value }}</p>
        <p v-if="stat.sub" class="text-xs text-content-tertiary dark:text-gray-500 mt-0.5">{{ stat.sub }}</p>
      </div>
    </div>
    ...
    <div class="px-8 pb-6 grid grid-cols-2 gap-6">
      <!-- Pie Chart -->
      <div class="card">
        <h3 class="text-sm font-semibold text-content dark:text-gray-100 mb-4">标签分布</h3>
        <div ref="pieChartRef" class="w-full h-72"></div>
      </div>

      <!-- Priority Pie Chart -->
      <div class="card">
        <h3 class="text-sm font-semibold text-content dark:text-gray-100 mb-4">优先级分布</h3>
    ...
    <div class="px-8 pb-6 grid grid-cols-2 gap-6">
      ...
      <div class="card">
        <h3 class="text-sm font-semibold text-content dark:text-gray-100 mb-4">每日任务数量</h3>
        <div ref="countChartRef" class="w-full h-72"></div>
      </div>

      <!-- Completion Rate Chart -->
      <div class="card">
        <h3 class="text-sm font-semibold text-content dark:text-gray-100 mb-4">完成率趋势</h3>

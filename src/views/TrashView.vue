<template>
  <div class="h-full flex flex-col overflow-y-auto">
    ...
    <div class="px-8 pt-6 pb-4 flex items-center justify-between">
      <div>
        <h1 class="text-2xl font-bold text-content dark:text-gray-100">回收站</h1>
    ...
    <div class="flex-1 px-8 pb-6">
      <!-- Empty -->
      <div v-if="store.trashTodos.length === 0" class="flex flex-col items-center justify-center h-64 text-content-tertiary dark:text-gray-500">
      ...
      <div v-else class="space-y-2">
        <div
          ...
          :key="todo.id"
          class="flex items-start gap-3 p-3 rounded-xl border border-border dark:border-gray-700 bg-surface dark:bg-gray-800 group"
        >
          <!-- Trash icon -->
          <div class="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
          ...
          <div class="flex-1 min-w-0">
            <p class="text-sm font-medium text-content dark:text-gray-200">{{ todo.title }}</p>
            <div class="flex items-center gap-2 mt-1 flex-wrap">
              <span class="text-xs text-content-tertiary dark:text-gray-500">
                原日期: {{ todo.todo_date }}
              </span>
              <span v-if="todo.deleted_at" class="text-xs text-content-tertiary dark:text-gray-500">
          ...
          <div class="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
            <button
              ...
              class="p-1.5 rounded-lg hover:bg-green-50 dark:hover:bg-green-900/20 text-content-tertiary hover:text-green-600 transition-colors"
              title="恢复"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <button
              ...
              class="p-1.5 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20 text-content-tertiary hover:text-red-500 transition-colors"
              title="永久删除"
            >
              <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" />
    ...
    <div v-if="deletingTodo" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-surface dark:bg-gray-800 rounded-2xl shadow-2xl border border-border dark:border-gray-700 p-6 w-80">
        <h3 class="text-lg font-semibold text-content dark:text-gray-100 mb-2">永久删除</h3>
        <p class="text-sm text-content-secondary dark:text-gray-400 mb-4">确定要永久删除「{{ deletingTodo.title }}」吗？此操作不可撤销。</p>
    ...
    <div v-if="confirmClearAll" class="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div class="relative bg-surface dark:bg-gray-800 rounded-2xl shadow-2xl border border-border dark:border-gray-700 p-6 w-80">
        <h3 class="text-lg font-semibold text-content dark:text-gray-100 mb-2">清空回收站</h3>
        <p class="text-sm text-content-secondary dark:text-gray-400 mb-4">确定要清空回收站中的所有 {{ store.trashTodos.length }} 个项目吗？此操作不可撤销。</p>
        <div class="flex justify-end gap-3">
...
function daysLeftClass(deletedAt) {
  ...
  const diff = Math.floor((now - deleted) / (1000 * 60 * 60 * 24))
  const remaining = 7 - diff
  if (remaining <= 1) return 'bg-red-100 dark:bg-red-900/30 text-red-600 dark:text-red-400'
  if (remaining <= 3) return 'bg-amber-100 dark:bg-amber-900/30 text-amber-600 dark:text-amber-400'

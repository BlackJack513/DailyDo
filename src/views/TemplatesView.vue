<template>
 ...
 <!-- Empty state -->
 <div v-if="templates.length === 0 && !loading" class="flex flex-col items-center justify-center py-20 text-content-tertiary">
...
>
 ...
 <button @click="editTemplate(tpl)" class="p-1.5 rounded-lg hover:bg-surface-tertiary text-content-tertiary hover:text-primary transition-colors" title="编辑">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" />
 </svg>
 </button>
 <button @click="confirmDelete(tpl)" class="p-1.5 rounded-lg hover:bg-red-50 hover:bg-red-50/20 text-content-tertiary hover:text-red-500 transition-colors" title="删除">
 <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
 ...
 <span v-if="tpl.recurrence_type && tpl.recurrence_type !== 'none'" class="text-xs px-2 py-0.5 rounded-full bg-blue-50 text-blue-600 text-blue-400 font-medium">
...
>🔒 {{ lf }}</span>
 ...
 <!-- Steps -->
 <div v-if="templateSteps[tpl.id] && templateSteps[tpl.id].length> 0" class="mt-2 pt-3 border-t border-border">
...
>
 ...
 <p v-if="templateSteps[tpl.id].length> 3" class="text-xs text-content-tertiary pl-6">+{{ templateSteps[tpl.id].length - 3 }} 更多...</p>
 </div>
 </div>
 </div>
 </div>
 ...
 :class="tplForm.priority === p.value ? p.activeClass : 'border-border text-content-tertiary hover:border-content-tertiary'"
>
 {{ p.label }}
 </button>
 </div>
...
>
 ...
 :class="tplForm.recurrence_type === r.value ? 'border-primary bg-primary/10 text-primary' : 'border-border text-content-tertiary hover:border-content-tertiary'"
>
 {{ r.label }}
 </button>
 </div>
 ...
 <label class="flex items-center gap-1.5 text-sm text-content-secondary text-muted cursor-pointer select-none">
 <input type="checkbox" value="title" v-model="tplForm.lockedFields" class="rounded border-border text-primary focus:ring-primary" />
 标题
 </label>
 <label class="flex items-center gap-1.5 text-sm text-content-secondary text-muted cursor-pointer select-none">
 <input type="checkbox" value="priority" v-model="tplForm.lockedFields" class="rounded border-border text-primary focus:ring-primary" />
 优先级
 </label>
 <label class="flex items-center gap-1.5 text-sm text-content-secondary text-muted cursor-pointer select-none">
 <input type="checkbox" value="tags" v-model="tplForm.lockedFields" class="rounded border-border text-primary focus:ring-primary" />
 标签
 </label>
 <label class="flex items-center gap-1.5 text-sm text-content-secondary text-muted cursor-pointer select-none">
 <input type="checkbox" value="recurrence" v-model="tplForm.lockedFields" class="rounded border-border text-primary focus:ring-primary" />
 重复
 </label>
 <label class="flex items-center gap-1.5 text-sm text-content-secondary text-muted cursor-pointer select-none">
 <input type="checkbox" value="steps" v-model="tplForm.lockedFields" class="rounded border-border text-primary focus:ring-primary" />
 步骤
 </label>
 </div>
 </div>
...
>
 ...
 <p class="text-sm text-content-secondary text-muted mb-5">确定要删除模板「{{ deleteTarget.name }}」吗？此操作不可恢复。</p>

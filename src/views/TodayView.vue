...
>
 ...
 badge-color="bg-green-100 text-green-700 bg-green-50/30 text-green-400"
 @drop="handleDrop"
 @toggle="handleToggle"
 @edit="handleEdit"
 @delete="handleDelete"
 ...
 <!-- Empty -->
 <div v-if="store.currentTodos.length === 0 && historicalGroups.length === 0" class="flex flex-col items-center justify-center h-64 text-content-tertiary">
 ...
 <button @click="showTemplatePicker = false" class="p-1 rounded-lg hover:bg-surface-tertiary text-content-tertiary">
...
>
 ...
 <span v-if="tpl.recurrence_type && tpl.recurrence_type !== 'none'" class="text-xs px-1.5 py-0.5 rounded bg-blue-50 text-blue-600 text-blue-400 font-medium">
 {{ tplRecurrenceLabel(tpl.recurrence_type) }}
 </span>
 <span v-if="getTplLockedCount(tpl)> 0" class="text-xs px-1.5 py-0.5 rounded bg-purple-50 bg-purple-50/20 text-purple-500 font-medium">
 ...
 <p class="text-sm text-content-secondary text-muted mb-4">确定要将「{{ deletingTodo.title }}」移入回收站吗？回收站中的项目将在 7 天后自动删除。</p>
...
>
 ...
 <div class="mt-0.5 flex-shrink-0 w-5 h-5 rounded-full border-2 flex items-center justify-center border-border"></div>
 <div class="flex-1 min-w-0">
